...
check_content_arithmetic и verify_content_consistency реально вызываются
в generate_and_save_lesson_new() (сессия 6 — в сессии 4 функции были
написаны и протестированы отдельно, но не подключены к точке входа; это
не было замечено до ручной вычитки в сессии 5).

Сессия 6 (2026-07-17): по решению владельца этап закрыт с текущими
цифрами (см. журнал сессий в CLAUDE.md) — оставшийся класс брака (словесные
...
_EXPR_EQUALS_RE = re.compile(
    rf'([{_ARITH_CHARSET}]*\d[{_ARITH_CHARSET}]*)\s*=\s*(-?\d+(?:[.,]\d+)?(?:/\d+)?)'
)
...
def check_content_arithmetic(content: LessonContent) -> list[dict]:
    ...
    урока (вызов 1) — например, "7 + 4 * 2 = 15" в объяснении. Правая часть
    может быть и дробью ("2/5 = 16/40" — равенство дробей, найдено сессией 6:
    без этого код ложно бракует любое сравнение эквивалентных дробей).
    Возвращает список найденных несовпадений (пусто = арифметика верна)."""
    problems: list[dict] = []
    for step in content.explain_steps:
        ...
        for m in _EXPR_EQUALS_RE.finditer(text):
            ...
            # group(1) может подхватить обычную пунктуацию перед числом
            # ("Например: 1/2" — двоеточие тоже в _ARITH_CHARSET, как синоним
            # деления) — обрезаем всё до первой (возможно знаковой) цифры,
            # иначе _eval_arith_chain получает "/1/2" и молча возвращает None
            # (найдено сессией 6 при проверке проверки: не ловил заведомо
            # неверные примеры именно из-за этого).
            lead = re.search(r'-?\d', expr_str)
            if lead:
                expr_str = expr_str[lead.start():]
            expected = _eval_arith_chain(expr_str)
            if expected is None:
                continue
            if '/' in claimed_str:
                num_str, denom_str = claimed_str.split('/', 1)
                denom = float(denom_str)
                if denom == 0:
                    continue
                claimed = float(num_str.replace(',', '.')) / denom
            else:
...
async def generate_and_save_lesson_new(
    ...
    content_arith_problems = check_content_arithmetic(content)
    if content_arith_problems:
        lesson.status = LessonStatus.needs_review
        lesson.validation_errors = [{
            "field": "content_arithmetic",
            "message": f"Вычислимая арифметика в тексте урока не сходится: {content_arith_problems[0]}",
            "severity": "critical",
        }]
        await db.flush()
        raise ValueError(
            f"Урок требует ручной проверки: арифметика в тексте не сходится "
            f"({len(content_arith_problems)} случаев)"
        )
    ...
    if not consistency.consistent:
        lesson.status = LessonStatus.needs_review
        lesson.validation_errors = [{
            "field": "content_consistency",
            "message": "; ".join(consistency.issues)[:500],
            "severity": "critical",
        }]
        await db.flush()
        raise ValueError(
            f"Урок требует ручной проверки: текст внутренне противоречив "
            f"({len(consistency.issues)} проблем)"
        )
